// Source - https://github.com/smithy-lang/smithy-typescript/blob/17b29f212802f4dde820409f93dffc8511660f89/packages/signature-v4/src/suite.fixture.ts
// License Apache 2.0 - https://github.com/smithy-lang/smithy-typescript/blob/17b29f212802f4dde820409f93dffc8511660f89/LICENSE

import { expect, it } from "vitest";
import { sigV4 } from "../src/sigv4.js";
...
const signingDate = new Date("2015-08-30T12:36:00Z");
...
for (const { name, request, authorization } of requests) {
  it(`should calculate the correct signature for ${name}`, async () => {
    expect((await sigV4({
      url: `${request.protocol}//${request.hostname}${request.path}?${Object.entries(request.query).map(p => p.map(a => encodeURIComponent(a as string)).join('=')).join('&')}`,
      ...
      signingDate,
      body: request.body
    })).authorization).toBe(authorization)
  });
}
