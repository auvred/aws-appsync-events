...
const arrayBufferHex: (buf: ArrayBuffer) => string =
  'toHex' in Uint8Array.prototype
...
async function sha256hex(data: string) {
  return arrayBufferHex(
    await crypto.subtle.digest(
      'SHA-256',
      (encoder ??= new TextEncoder()).encode(data),
    ),
  )
}
...
export async function sigV4(opts: SigV4Opts): Promise<{
  ...
  const headers = Object.keys(opts.headers)
    .map(
      k =>
        [
          k.toLowerCase(),
          opts.headers[k]!.replace(/^s+|s+$/g, '').replace(/\s+/g, ' '),
        ] as const,
    )
  ...
  const datetime = (opts.signingDate ?? new Date())
    .toISOString()
    .replace(/[:-]|\.\d{3}/g, '')
  ...
  const signingKey = await hmac(
    await hmac(
      await hmac(await hmac(`AWS4${opts.secretAccessKey}`, date), opts.region),
      opts.service,
    ),
    'aws4_request',
  )
